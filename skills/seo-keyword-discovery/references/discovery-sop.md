...
After the first-round source collection and Candidate analysis, freeze the Root/Natural Seeds handoff as a `seo-discovery-input/v1` `discovery_input_manifest` before Coverage or low-risk cleaning. The manifest records the original Seed total/list, the original observed Candidate inventory, and the complete analysis state for every Candidate. Obtain and retain its production validation receipt; the later Coverage ledger must match it exactly.
