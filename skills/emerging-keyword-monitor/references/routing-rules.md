...
That handoff enters `seo-keyword-selection` **directly**. It must not restart keyword discovery for the same confirmed candidate and must never route through `seo-keyword-discovery`. Once confirmed and handed off, never rerun Seed generation, Google Autocomplete, or Semrush Ideas for the same handoff merely to re-discover it.
